import { LinkPrecedence, Prisma, type User } from "../generated/prisma/client";
...
export const turnOldestToPrimary = async (
  ...
  const r1 =
    u1.linkPrecedence === LinkPrecedence.PRIMARY
      ? u1
      : await tx.user.findUnique({ where: { id: u1.linkedId! } });
  const r2 =
    u2.linkPrecedence === LinkPrecedence.PRIMARY
      ? u2
      : await tx.user.findUnique({ where: { id: u2.linkedId! } });
